...
We have Git, Python, and VSCode installed. \texttt{python {-}{-}version} gives
\texttt{Python 3.12.4} as the output. \texttt{git {-}{-}version} gives
...
based on our criteria. We use CodeQL, Semgrep and Snyk to analyze the chosen repositories.
\begin{figure}[!ht]
    \centering
...
command line (Fig.~\ref{1_3}). For snyk, we installed it via \texttt{npm}, and then set up
...
using \texttt{./gradlew build} command (Fig.~\ref{1_8}).
...
For two sets $x$, $y$, the IoU is given as:
...
\subsubsection{Overall IoU metrics}
...
\begin{table}[!ht]
    ...
    \begin{tabular}{lccc}
        ...
        \hline
    \end{tabular}
    \caption{IoU over all unique CWEs detected by the three tools.}
\end{table}

This shows that CodeQL and Semgrep or CodeQL and Snyk have very diverse detection capabilities. However, 31\% of vulnerabilities detected by Snyk and Semgrep are the same. So there is partial redundancy for Snyk and Semgrep. The venn diagram below visualizes this (Fig.~\ref{1_14}).

\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.5\linewidth]{figures/1_14.png}
    \caption{Venn diagram for overall coverage.}
    \label{1_14}
\end{figure}
...
tools. Our analysis of the CWE Top 25 coverage showed that Snyk had the
...
CodeQL finds highly unique vulnerabilities, whereas there is some redundancy between Semgrep and Snyk.

To maximize CWE coverage, a combination of tools is necessary. Based on our
findings, using CodeQL and Snyk (Semgrep also works but it detects lower number of vulnerabilities than Snyk) together would provide the best coverage for
